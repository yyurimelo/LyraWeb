  ...
  return {
    ...
    pages: old.pages.map((page: any) => ({
      ...
      data: page.data.filter(
        (n: any) => String(n.id) !== String(notificationId),
      ),
...
const updateNotificationStatusInCache =
  (referenceId: number, status: string) => (old: any) => {
    if (!old || !old.pages) return old;
    return {
      ...old,
      pages: old.pages.map((page: any) => ({
  ...
  useEffect(() => {
    ...
    connection.on("notificationreceived", (notification: any) => {
      ...
      const mappedNotification = {
        ...
        receiverId:
          notification.receiverId?.toString() || notification.receiverId,
      ...
      if (mappedNotification.status === "unread") {
        queryClient.setQueryData(
          ...
          (old: any) => {
            ...
            const exists = old.data.some(
              (n: any) => n.id === mappedNotification.id,
            );
            if (exists) return old;
        ...
        queryClient.setQueryData(
          ["notifications", "infinite", { status: undefined, type: "" }],
          addNotificationToCache(mappedNotification),
        );
        queryClient.setQueryData(
          ["notifications", "infinite", { status: false, type: "" }],
          addNotificationToCache(mappedNotification),
        );
        if (mappedNotification.type === "InviteFriend") {
          queryClient.setQueryData(
            [
              "notifications",
              "infinite",
              { status: undefined, type: "InviteFriend" },
            ],
            addNotificationToCache(mappedNotification),
          );
          queryClient.setQueryData(
            [
              "notifications",
              "infinite",
              { status: false, type: "InviteFriend" },
            ],
            addNotificationToCache(mappedNotification),
          );
          ...
          queryClient.setQueryData(
            [
              "notifications",
              "infinite",
              { status: undefined, type: "AcceptFriendRequest" },
            ],
            addNotificationToCache(mappedNotification),
          );
          queryClient.setQueryData(
            [
              "notifications",
              "infinite",
              { status: false, type: "AcceptFriendRequest" },
            ],
            addNotificationToCache(mappedNotification),
          );
          ...
          queryClient.setQueryData(
            [
              "notifications",
              "infinite",
              { status: undefined, type: "NewMessage" },
            ],
            addNotificationToCache(mappedNotification),
          );
          queryClient.setQueryData(
          ...
          queryClient.setQueryData(
            [
              "notifications",
              "infinite",
              { status: undefined, type: "System" },
            ],
            addNotificationToCache(mappedNotification),
          );
          queryClient.setQueryData(
            ["notifications", "infinite", { status: false, type: "System" }],
            addNotificationToCache(mappedNotification),
          );
        }
      } else {
        queryClient.setQueryData(
          ["notifications", "header", "read"],
          (old: any) => {
            if (!old) {
              return {
                data: [mappedNotification],
        ...
        queryClient.setQueryData(
          ["notifications", "infinite", { status: true, type: "" }],
          addNotificationToCache(mappedNotification),
        );
        if (mappedNotification.type === "InviteFriend") {
          queryClient.setQueryData(
          ...
          queryClient.setQueryData(
            [
              "notifications",
              "infinite",
              { status: true, type: "AcceptFriendRequest" },
            ],
            addNotificationToCache(mappedNotification),
          );
        } else if (mappedNotification.type === "NewMessage") {
          queryClient.setQueryData(
            ["notifications", "infinite", { status: true, type: "NewMessage" }],
            addNotificationToCache(mappedNotification),
          );
        } else if (mappedNotification.type === "System") {
          queryClient.setQueryData(
            ["notifications", "infinite", { status: true, type: "System" }],
            addNotificationToCache(mappedNotification),
          );
        }
      }
    ...
    connection.on("notificationremoved", (notificationId: number) => {
      ...
      queryClient.setQueryData(
        ["notifications", "infinite", { status: false, type: "InviteFriend" }],
        removeNotificationFromCache(notificationId),
      );
      queryClient.setQueryData(
        [
      ...
      queryClient.setQueryData(
        ["notifications", "infinite", { status: false, type: "NewMessage" }],
        removeNotificationFromCache(notificationId),
      );
      queryClient.setQueryData(
        ["notifications", "infinite", { status: undefined, type: "System" }],
        removeNotificationFromCache(notificationId),
      );
      queryClient.setQueryData(
        ["notifications", "infinite", { status: false, type: "System" }],
    ...
    connection.on(
      ...
      (payload: { referenceId: number; status: string }) => {
        ...
        queryClient.setQueryData(
          ["notifications", "infinite", { status: undefined, type: "" }],
          updateNotificationStatusInCache(payload.referenceId, payload.status),
        );
        queryClient.setQueryData(
          ["notifications", "infinite", { status: false, type: "" }],
          updateNotificationStatusInCache(payload.referenceId, payload.status),
