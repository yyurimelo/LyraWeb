...
function NotificationsPage() {
  ...
  const { mutateAsync: maskAsReadFn, isPending: isMarkingAsRead } =
    useMaskAsReadMutation();
  ...
  const showFriendRequestUI =
    filterType !== "messages" && filterType !== "system";
        ...
        <main className="flex-1 min-w-0 border rounded-md p-4 space-y-4">
          ...
          {showFriendRequestUI ? (
            ...
            <div className="flex items-center justify-center py-12">
              <p className="text-muted-foreground">
                {filterType === "messages"
                  ? `${t("notifications.filters.messages")} - ${t("notifications.comingSoon")}`
                  : `${t("notifications.filters.system")} - ${t("notifications.comingSoon")}`}
              </p>
            </div>
          )}
