...
export const FriendRequestItem = memo(
  ({ request, onActionSuccess }: FriendRequestItemProps) => {
    const { t, i18n } = useTranslation();
    const { mutate: acceptRequest, isPending: isAcceptingMutation } =
      useAcceptFriendRequestMutation();
    const { mutate: cancelRequest, isPending: isCancelingMutation } =
      useCancelFriendRequestMutation();
      ...
      <div className="flex items-start gap-3 p-3 bg-accent/25 border cursor-pointer rounded-md last:border-b-0 hover:bg-muted/30 transition-colors">
        <Avatar className="size-11 rounded-full transition-transform">
          ...
          <AvatarFallback
            style={{
              backgroundColor:
                request.senderPrimaryColor || "hsl(var(--primary))",
            }}
